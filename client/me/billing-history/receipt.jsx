...
const BillingReceipt = React.createClass( {
	...
	renderLineItems() {
		const { translate, transaction } = this.props;
		const creditBadge = <span className="billing-history__credit-badge">{ translate( 'Refund' ) }</span>;
		const items = transaction.items.map( function( item ) {
			return (
				<tr key={ item.id }>
